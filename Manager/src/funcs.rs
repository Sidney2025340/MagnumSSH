...
pub fn generate_test(time: usize, conn: &Connection) -> String {
    ...
    let commands = [
        ...
        format!("(echo {}; echo {}) | passwd {}", pass, pass, user),
        format!("echo \"/opt/magnumssh/manager --remove-user {}\" | at \"now + {} minute\"", user, time),
    ];
...
pub fn enable_proxy_port(port: String, status: String) {
    if status.is_empty() {
        run_command(format!("/opt/magnumssh/connectionsmanager --conn proxy --enable-port {}", port));
    } else {
        run_command(format!("/opt/magnumssh/connectionsmanager --conn proxy --enable-port {} --status {}", port, status));
    }
}

pub fn disable_proxy_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn proxy --disable-port {}", port));
}



pub fn enable_sslproxy_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn sslproxy --enable-port {}", port));
}
pub fn disable_sslproxy_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn sslproxy --disable-port {}", port));
}

pub fn enable_badvpn_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn badvpn --enable-port {}", port));
}

pub fn disable_badvpn_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn badvpn --disable-port {}", port));
}

pub fn enable_checkuser_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn checkuser --enable-port {}", port));
}

pub fn disable_checkuser_port(port: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn checkuser --disable-port {}", port));
}

pub fn enable_openvpn(port: String, mode: String) {
    run_command(format!("/opt/magnumssh/connectionsmanager --conn openvpn --enable {} --mode {}", port, mode));
}

pub fn disable_openvpn() {
    run_command("/opt/magnumssh/connectionsmanager --conn openvpn --disable".to_string());
}
