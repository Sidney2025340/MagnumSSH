...
if [ "$EUID" -ne 0 ]; then
    ...
    show_progress "Criando diretorio /opt/magnumssh..."
    mkdir /opt/ > /dev/null 2>&1
    mkdir /opt/magnumssh > /dev/null 2>&1
    increment_step

    # ---->>>> Criando as colunas no banco de dados
    show_progress "Configurando o banco de dados..."
    sqlite3 /opt/magnumssh/db "
    ...
    for column in "proxy_ports" "sslproxy_ports" "badvpn_ports" "checkuser_ports" "openvpn_port"; do
        column_exists=$(sqlite3 /opt/magnumssh/db "PRAGMA table_info(connections);" | grep -w "$column" | wc -l)
        if [ "$column_exists" -eq 0 ]; then
            sqlite3 /opt/magnumssh/db "ALTER TABLE connections ADD COLUMN $column TEXT;"
        fi
    done
    if [ $? -ne 0 ]; then
    ...
    mkdir -p /root/MagnumSSH/BadVpn/src/badvpn-build
    cd /root/MagnumSSH/BadVpn/src/badvpn-build
    ...
    mv -f udpgw/badvpn-udpgw /opt/magnumssh/badvpn
    increment_step
