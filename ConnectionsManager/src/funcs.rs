...
pub struct Connections {
    pub(crate) proxy: MagnumProxy,
    pub(crate) sslproxy: MagnumProxySSL,
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct MagnumProxy {
    pub(crate) ports: Option<String>,
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct MagnumProxySSL {
    pub(crate) ports: Option<String>,
}
...
pub fn add_proxy_port(port: usize, status: Option<String>) -> Result<(), io::Error> {
    ...
    let mut command = format!("/opt/magnumssh/magnumproxy --port {}", port);
    if status.is_some() {
        command = format!("{} --status {}", command, status.unwrap_or("@MagnumProxy".to_string()));
    }
    let service_file_content = format!(r#"
[Unit]
Description=MagnumProxy{}
...
"#, port, command);
    ...
    let service_file_path = format!("/etc/systemd/system/magnumproxy{}.service", port);
    ...
    let commands = [
        ...
        format!("systemctl enable magnumproxy{}.service", port),
        format!("systemctl start magnumproxy{}.service", port),
    ];
...
pub fn del_proxy_port(port: usize) -> Result<(), io::Error> {
    let commands = [
        format!("systemctl disable magnumproxy{}.service", port),
        format!("systemctl stop magnumproxy{}.service", port),
    ];
    for command in commands {
        run_command(command);
    }
    fs::remove_file(format!("/etc/systemd/system/magnumproxy{}.service", port))?;
    Ok(())
}

pub fn add_ssl_proxy_port(port: usize, cert: Option<&String>, key: Option<&String>) -> Result<(), io::Error> {

    let mut command = format!("/opt/magnumssh/magnumproxyssl --proxy-port {}", port);
    if cert.is_some() {
...
"#, port, command);
    ...
    let service_file_path = format!("/etc/systemd/system/magnumproxyssl{}.service", port);
    ...
    let commands = [
        "systemctl daemon-reload".to_string(),
        format!("systemctl enable magnumproxyssl{}.service", port),
        format!("systemctl start magnumproxyssl{}.service", port),
    ];
    for command in commands {
        run_command(command);
    }
...
pub fn del_ssl_proxy_port(port: usize) -> Result<(), io::Error> {
    let commands = [
        format!("systemctl disable magnumproxyssl{}.service", port),
        format!("systemctl stop magnumproxyssl{}.service", port),
    ];
    for command in commands {
        run_command(command);
    }
    fs::remove_file(format!("/etc/systemd/system/magnumproxyssl{}.service", port))?;
    Ok(())
}
...
Restart=always
Type=simple
ExecStart=/opt/magnumssh/badvpn --listen-addr [::]:{} --max-clients 1000 --max-connections-for-client 1000 --client-socket-sndbuf 0 --udp-mtu 9000
...
Restart=always
Type=simple
ExecStart=/opt/magnumssh/checkuser --port {}
